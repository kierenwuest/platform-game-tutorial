...
class Layer { // classes start with capital and are like a blueprint, for similar qualites with different values. Require a mandatory construtor method()
    ...
    update(){ // and update method in the class which updates values
        this.speed = globalGameSpeed * this.speedMod; 
        if (this.x <= -this.width) { // this is detecting the edge and reset to 0
            this.x = 0;
        }
    ...
    draw(){ // draws stuff on the canvas
        ctx.drawImage(this.image, this.x, this.y + this.yOffset , this.width+2 , this.height)
        ctx.drawImage(this.image, this.x + this.width , + this.yOffset , this.width , this.height) // this replaces the paris of draw image below
    }
}
...
const gameObjects = [layer11,layer10,layer9,layer8,layer7,layer6,layer5,layer4,layer3,layer2,layer1,layer0]; // the order of this needs to be right to left
...
function animationLoop () {
    ...
    gameObjects.forEach(object => { //!! calls the update and draws from each array class object!
        object.update();
        object.draw();
    });
